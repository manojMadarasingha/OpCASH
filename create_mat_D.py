...
# store the matrix A
def store_matrix_D(df_ct, store_path, n):
    user_path = store_path + '/user_' + str(n)
    if not os.path.exists(user_path):
        os.makedirs(user_path)

    df_ct.to_csv(user_path + '/D.csv', index=False)

    return


def create_matrix_D(vp, n, store_path, cached_tiles, ena_store):
    ...
    if ena_store:
        ...
        store_matrix_D(df_A, store_path, n)
